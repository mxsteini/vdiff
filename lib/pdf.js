...
module.exports = (templatesDir, data, templateHelper) => {
  return {
    ...
    async create (configuration, browserName, sequences, tempDir, options) {
      let pages = []
      for (let sequence of sequences) {
        better.info('collect pages of: ' + sequence)
        ...
        if (!!configuration['sequences'][sequence]['initialActions']) {
          if (configuration['sequences'][sequence]['initialActions'].path) {
            let stepCounter = 0
            for (let singleTest of configuration['sequences'][sequence]['initialActions']['steps']) {
        ...
        better.info('writing pdf: ' + sequence)
        await this.renderPdf(templatesDir, pages, path.join(workDir, sequence))
        better.info('finished pdf in: ' + path.join(workDir, sequence) + '.pdf')
      }
    }
