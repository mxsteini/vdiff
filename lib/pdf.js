...
const renderPdf = async (templatesDir, content, output) => {
...
module.exports = (templatesDir, data, templateHelper) => {
  return {
    async create (configuration, browserName, domains, tempDir, options) {
      let pages = []
      for (let domain of domains) {
        ...
        if (!!configuration['sequences'][domain]['initialActions']) {
          if (configuration['sequences'][domain]['initialActions'].path) {
            let stepCounter = 0
            for (let singleTest of configuration['sequences'][domain]['initialActions']['steps']) {
        ...
        await renderPdf(templatesDir, pages, path.join(workDir, domain))
        better.info('finished pdf in: ' + path.join(workDir, domain) + '.pdf')
      }
    }
