...
module.exports = (configuration, options, templateHelper) => {
  ...
  return {
    create(browser, browserNames, sequencesName, tempDir, q) {
      console.log('start sequence: ' + sequencesName)
      ...
      for (let browserName of configuration.sequences[sequencesName].browser) {
        const puppeteerConfig = {
          ...
          args: configuration.setup.broswerArgs || [],
          defaultViewport: {
          ...
          .then(async result => {
              q.push(async function () {
                ...
                if (!configuration.sequences[sequencesName].target[target1]) {
                  console.log(sequencesName + '-' + browserName + ': Target ' + target1 + ' not found in configuration')
                  process.exit(1)
                }

                if (!configuration.sequences[sequencesName].target[target2]) {
                  console.error(sequencesName + '-' + browserName + ': Target ' + target2 + ' not found in configuration')
                  process.exit(1)
                }
                ...
                for (let target of processTargets) {
                  browser[sequencesName][browserName][target.target] = await browser[sequencesName][browserName].browser.createBrowserContext()
                }
                ...
                for (let singleTest of configuration['sequences'][sequencesName]['list']) {
                  ...
                  console.log(sequencesName + '-' + browserName + ': adding test')
                ...
                console.log(sequencesName + '-' + browserName + ': waiting for queue')
              })
            }
          )
