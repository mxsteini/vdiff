      ...
      puppeteer.launch(puppeteerConfig)
        .then(async result => {
            ...
            for (let sequence of sequences) {
              ...
              if (!configuration['sequences'][sequence].target && !!configuration.default.target) {
                configuration['sequences'][sequence].target = configuration.default.target
              }

              if (!configuration['sequences'][sequence]['target'][options.target1]) {
                better.error('Target ' + options.target1 + ' not found in configuration')
                process.exit(1)
              }

              if (!configuration['sequences'][sequence]['target'][options.target2]) {
              ...
              if (options.skipTarget !== '1') {
                processTargets.push({
                  url: configuration['sequences'][sequence]['target'][options.target1],
                  target: options.target1
                })
              }
              if (options.skipTarget !== '2') {
                processTargets.push({
                  url: configuration['sequences'][sequence]['target'][options.target2],
              ...
              if (!!configuration['sequences'][sequence]['initialActions']) {
                if (configuration['sequences'][sequence]['initialActions'].path) {
                  await funcs.processTarget(browser, browserName, sequence, workDir, processTargets, 'initial', configuration['sequences'][sequence]['initialActions'])
                }
              }
              ...
              for (let singleTest of configuration['sequences'][sequence]['list']) {
                ...
                q.push(async function () {
                  ...
                  return await funcs.processTarget(browser, browserName, sequence, workDir, processTargets, filename, test)
                })
              }
