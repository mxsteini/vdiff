...
module.exports = (options, configuration) => {
  return {
    ...
    async processTarget (browser, browserName, sequence, workDir, processTargets, filename, test) {
      ...
      better.info('starting Tests: ' + browserName + ' ' + sequence + ' ' + filename)
      ...
      for (let step of test.steps) {
        let stepName = filename + '_' + (stepCounter)
        ...
        let screenshot1 = path.join(workDir, options.target1, stepName + '.png')
        let screenshot2 = path.join(workDir, options.target2, stepName + '.png')
