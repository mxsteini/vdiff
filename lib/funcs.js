...
module.exports = (options, configuration) => {
  return {
    ...
    async processTarget (browser, browserName, sequence, workDir, processTargets, filename, test) {
      const page = []
      let promiseCollector = []
      let stepCounter = 0
      let screenshots = []

      better.info('starting Tests: ' + browserName + ' ' + sequence)
      ...
      for (let step of test.steps) {
        ...
        let screenshot1 = path.join(workDir, options.target1, filename + '_' + (stepCounter) + '.png')
        let screenshot2 = path.join(workDir, options.target2, filename + '_' + (stepCounter) + '.png')
        ...
        for (let target of processTargets) {
          ...
          screenshots[target.target] = await this.processAction(page[target.target], step, filePath, configuration.browser[browserName].height, options)
            .catch(() => better.error('process target loop: ' + filename + '_' + (stepCounter)))
        }
        ...
        await Promise.all([
          ...
          .then(images => {
            if (typeof screenshots[options.target1] == 'object') {
              images[0].writeAsync(screenshot1)
                .catch(() => better.error('error writing screenshot 1 :' + screenshot1))
            }
            if (typeof screenshots[options.target1] == 'object') {
              images[1].writeAsync(screenshot2)
                .catch(() => better.error('error writing screenshot 2: ' + screenshot2))
            }
            let diffImage = path.join(workDir, 'diff', filename + '_' + (stepCounter) + '.png')
            return jimp.diff(images[0], images[1]).image.writeAsync(diffImage)
              .catch(() => better.error('error creating diff for ' + diffImage))
          })
          .catch(() => better.error('error in Promise.all diff for ' + filename + '_' + (stepCounter)))
        stepCounter++
      }
      for (let target of processTargets) {
