...
module.exports = (options, configuration) => {
  return {
            ...
            case 'hover':
              await page.hover(step.action_selector)
                .catch(err => {
                  ...
                  console.error('processStep: ' + step.action)
                })
              break
            case 'click':
              await page.click(step.action_selector)
                .catch(err => {
                  ...
                  console.error('url: ' + page.url())
                  console.error('processStep: ' + step.action)
                  console.error('selector: ' + step.action_selector)
                })
              break
            case 'focus':
    ...
    async processTarget(browser, browserName, sequenceName, workDir, processTargets, testName, test, close = false) {
      ...
      console.log(sequenceName + '-' + browserName + ': starting Tests: ' + browserName + ' ' + sequenceName + ' ' + testName + ' steps: ' + test.steps.length)
      for (let target of processTargets) {
        ...
        await page[target.target].goto(targetUrl, {
          'timeout': configuration.setup.timeout || 3000,
          'waitUntil': configuration.setup.waitUntil || 'networkidle0'
        })
          .catch((e) => {
            console.error('error in page.goto: ' + targetUrl)
      ...
      for (let step of test.steps) {
        ...
        console.log(sequenceName + '-' + browserName + ': starting step ' + stepName)
          ...
          await Promise.all([
            ...
            .then(images => {
              ...
              return jimp.diff(images[0], images[1]).image.writeAsync(diffImage)
                .catch(() => console.error('error creating diff for ' + diffImage))
            })
            .catch(() => console.error('error in Promise.all diff for ' + stepName))
        }
        console.log(sequenceName + '-' + browserName + ': finish step ' + stepName)
        stepCounter++
      }
      ...
      if (close) {
        ...
        await Promise.all(promiseCollector)
          .catch((err) => {
            console.error('error in closing pages')
            console.error('funcs: 145', err.message)
          })
      }
    }
  }
