            ...
            case 'hover':
              await page.hover(step.action_selector)
                .catch(err => {
                  ...
                  better.error('url: ' + page.url())
                  better.error('processStep: ' + step.action)
                })
              break
            case 'click':
              await page.click(step.action_selector)
                .catch(err => {
                  ...
                  better.error('url: ' + page.url())
                  better.error('processStep: ' + step.action)
                  better.error('selector: ' + step.action_selector)
                })
              break
            case 'focus':
      ...
      if (close) {
        ...
        await Promise.all(promiseCollector)
          .catch((err) => {
            ...
            better.error('funcs: 145', err.message)
          })
      }
    }
  }
