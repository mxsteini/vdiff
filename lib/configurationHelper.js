...
module.exports = (projectDir, resourcesDir) => {
  return {
    configuration () {
      ...
      try {
        ...
        better.error('cp configuration from template')
        fs.copyFileSync(path.join(resourcesDir, 'misc', 'configuration.json.dist'), path.join(projectDir, 'configuration.json'))
        better.error('cp tests from template')
