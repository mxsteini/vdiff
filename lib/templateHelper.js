...
module.exports = (configuration, options, projectDir, diffToolDir, data) => {
  ...
  return {
    ...
    createDiffList (tempDir, templatesDir, browserNames, data, sequencesNames) {
      ...
      for (let sequencesName of sequencesNames) {
        for (let browserName of configuration.sequences[sequencesName].browser) {
          ...
          let targets = funcs.getTargets(sequencesName)
          ...
          if (initialActions !== null) {
            ...
            for (let singleTest of initialActions) {
              ...
              let target1url = (test.url ? test.url : targets[target1].url)
              let target2url = (test.url ? test.url : targets[target2].url)
              ...
              for (let step of test.steps) {
                if (step.screenshot !== false) {
                  diffList.initials.push({
                    ...
                    diffHtml: path.join('Html', sequencesName, browserName, 'html', filename + '_' + (stepCounter) + '.html'),
                    diffImage: path.join('Html', sequencesName, browserName, 'diff', filename + '_' + (stepCounter) + '.png')
                  })
                  let diffHTML = Mustache.render(diffTemplate, {
                    ...
                    target1: target1,
                    target2: target2,
                    image1: path.join('Html', sequencesName, browserName, target1, filename + '_' + (stepCounter) + '.png'),
                    image2: path.join('Html', sequencesName, browserName, target2, filename + '_' + (stepCounter) + '.png'),
                    data: data
                  })
