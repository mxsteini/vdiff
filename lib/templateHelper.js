    ...
    createDiffList (tempDir, templatesDir, browsers, data, sequences) {
      ...
      for (let browserName of browsers) {
        for (let sequence of sequences) {
          let target1 = options.target1 || configuration.sequences[sequence].target1
          let target2 = options.target2 || configuration.sequences[sequence].target2
              ...
              for (let step of test.steps) {
                if (step.screenshot !== false) {
                  ...
                  let diffHTML = Mustache.render(diffTemplate, {
                    ...
                    target1: target1,
                    target2: target2,
                    image1: path.join('Html', sequence, browserName, target1, filename + '_' + (stepCounter) + '.png'),
                    image2: path.join('Html', sequence, browserName, target2, filename + '_' + (stepCounter) + '.png'),
                    data: data
                  })
